...
status in addition to `draft`, `accepted`, `superseded`, and `rejected`.
...
event, or link. Typed direct lookups are also available where useful, including
`memory(id)`, `task(id)`, `decision(id)`, `artifact(id)`, `event(id)`, and
`link(id)`.
